...
if (a[i].indexOf('FILENAME') > -1 )      	b5  = a[i];
						b5index = b5.lastIndexOf(">")+1;
						b5  = b5.substring(0, b5index);
if (a[i].indexOf('Content Contributor') > -1 
	|| a[i].indexOf('Created') > -1 ) 	b6  = a[i];
						b6index = b6.lastIndexOf(">")+1;
						b6  = b6.substring(0, b6index);
//if (a[i].indexOf('Project')> -1 )        	b7  = a[i];//calculate admin db institution
						b7index = b7.lastIndexOf(">")+1;
						b7  = b7.substring(0, b7index);
if (a[i].indexOf('Date:') > -1 
|| a[i].indexOf('Timestamp:')>-1 )         	b8  = a[i];
						b8index = b8.lastIndexOf(">")+1;
						b8  = b8.substring(0, b8index);
//if(a[i].indexOf('NUMBER_OF_CONSTITUENT')>-1)  b9  = a[i]; always calculate
						b9index = b9.lastIndexOf(">")+1;
						b9  = b9.substring(0, b9index);
//if (a[i].indexOf('Reload')>-1 || a[i].indexOf('RELOAD') > -1 ) b10 = a[i];//this function is called by reload, always calculate
if (a[i].indexOf('Reload')>-1 || a[i].indexOf('RELOAD') > -1 ) silent_reload = a[i];//use unchanged value for silent_reload
						sRindex = silent_reload.lastIndexOf(">")+1;
						silent_reload  = silent_reload.substring(0, sRindex);
...
if (parent.U.silent_reload_num != "2") {
b10 = "<!-- RELOAD: "+DATE+" -->";
parent.U.document.UF.VVcomment_03.value = b10;
}else{
parent.U.document.UF.VVcomment_03.value = silent_reload;
alert("silent reload");
}
...
if ( testXMLBASE != parent.adb_admindb_data ) {
	...
	+"\n\n========================================="
	+"\n\nDo you wish to continue anyway? Click OK.") ) {
	parent.U.document.UF.reset();
	return(0);
	}else {
	parent.adb_admindb_data = testXMLBASE;
...
//because the name-type radio buttons are not same for name_01, name_02, name_03
		...
		//alert("b2="+b2);
			var complex_p_name=0;
			//Complex Personal Name
			if (b2.indexOf('type')<0 && b2.indexOf('anonymous')<0 && b2.indexOf('unknown')<0) {
				//alert("this is a complex personal name");
				complex_p_name=1;
				}
		...
		if (complex_p_name == 1) {
			eval('parent.U.document.UF.creator_complexname_0'+N+'.value = getEntityData(b2);');
			eval('parent.U.document.UF.creator_lastname_0'+N+'.value = "";');
		}else{
			eval('parent.U.document.UF.creator_complexname_0'+N+'.value = "";');
