...
function dataTuple_11 (a,b,c,d,e,f,g,h,i,j,k) {
this.a = a;
this.b = b;
this.c = c;
this.d = d;
...
testData2();
alert("mods_header=\n\n"+mods_header);
extract_page_begin(mods_header); 
...
var MEB_cursor=0;
    ...
    mods_titleInfo=get_MODS_element_block2('<mods:titleInfo','</mods:titleInfo>','<mods:name',chompStr,MEB_cursor);
    MEB_cursor += mods_titleInfo.length;
    mods_name_01  =get_MODS_element_block2('<mods:name type','</mods:name>','<mods:originInfo',chompStr,MEB_cursor);
    MEB_cursor += mods_name_01.length;
    mods_name_02  =get_MODS_element_block2('<mods:name type','</mods:name>','<mods:originInfo',chompStr,MEB_cursor);
    MEB_cursor += mods_name_02.length;
    mods_name_03  =get_MODS_element_block2('<mods:name type','</mods:name>','<mods:originInfo',chompStr,MEB_cursor);
    MEB_cursor += mods_name_03.length;
    mods_originInfo = get_MODS_element_block2('<mods:originInfo','</mods:originInfo>','<mods:language',chompStr,MEB_cursor);
    MEB_cursor += mods_originInfo.length;
    mods_language = get_MODS_element_block2('<mods:language','</mods:language>','<mods:typeOfResource',chompStr,MEB_cursor);
    MEB_cursor += mods_language.length;
    mods_typeOfResource_01 = get_MODS_element_block2('<mods:typeOfResource','</mods:typeOfResource>','<mods:genre',chompStr,MEB_cursor);
    MEB_cursor += mods_typeOfResource_01.length;
    mods_typeOfResource_02 = get_MODS_element_block2('<mods:typeOfResource','</mods:typeOfResource>','<mods:genre',chompStr,MEB_cursor);
    MEB_cursor += mods_typeOfResource_02.length;
    mods_typeOfResource_03 = get_MODS_element_block2('<mods:typeOfResource','</mods:typeOfResource>','<mods:genre',chompStr,MEB_cursor);
    MEB_cursor += mods_typeOfResource_03.length;
    mods_genre = get_MODS_element_block2('<mods:genre authority','</mods:genre>','<mods:physicalDescription',chompStr,MEB_cursor);
    MEB_cursor += mods_genre.length;
    mods_physicalDescription = get_MODS_element_block2('<mods:physicalDescription','</mods:physicalDescription>','<mods:abstract',chompStr,MEB_cursor);
    MEB_cursor += mods_physicalDescription.length;
    mods_abstract = get_MODS_element_block2('<mods:abstract','</mods:abstract>','<mods:subject',chompStr,MEB_cursor);
    MEB_cursor += mods_abstract.length;
    mods_subject_auth_lcsh = get_MODS_element_block2('<mods:subject authority="lcsh"','</mods:subject>','<mods:relatedItem',chompStr,MEB_cursor);
    MEB_cursor += mods_subject_auth_lcsh.length;
    mods_subject_auth_tgm = get_MODS_element_block2('<mods:subject authority="tgm"','</mods:subject>','<mods:relatedItem',chompStr,MEB_cursor);
    MEB_cursor += mods_subject_auth_tgm.length;
    mods_subject_auth_vv = get_MODS_element_block2('<mods:subject authority="vv"','</mods:subject>','<mods:relatedItem',chompStr,MEB_cursor);
    MEB_cursor += mods_subject_auth_vv.length;
    mods_subject_auth_spc = get_MODS_element_block2('<mods:subject authority="spc"','</mods:subject>','<mods:relatedItem',chompStr,MEB_cursor);
    MEB_cursor += mods_subject_auth_spc.length;
    mods_subject_auth_none = get_MODS_element_block2('<mods:subject ','</mods:subject>','<mods:relatedItem',chompStr,MEB_cursor);
    MEB_cursor += mods_subject_auth_none.length;
    mods_constituent_files = get_MODS_element_block2('<mods:relatedItem type="constituent"','</mods:relatedItem','<mods:location',chompStr,MEB_cursor);
    MEB_cursor += mods_constituent_files.length;
    mods_uri = get_MODS_element_block2('<mods:identifier type="uri"','</mods:identifier>','<mods:location',chompStr,MEB_cursor);
    MEB_cursor += mods_uri.length;
    mods_location = get_MODS_element_block2('<mods:location','</mods:location>','<mods:note',chompStr,MEB_cursor);
    MEB_cursor += mods_location.length;
    mods_note = get_MODS_element_block2('<mods:note type','</mods:note>','<mods:recordInfo',chompStr,MEB_cursor);
    MEB_cursor += mods_note.length;
    mods_recordInfo = get_MODS_element_block2('<mods:recordInfo','</mods:recordInfo>','<mods:accessCondition',chompStr,MEB_cursor);
    MEB_cursor += mods_recordInfo.length;
    mods_accessCondition = get_MODS_element_block2('<mods:accessCondition','</mods:accessCondition>','</mods:mods>',chompStr,MEB_cursor);
    MEB_cursor += mods_accessCondition.length;

alert("testData2: mods elements are complete");
...
var MEB_cursor=0;
    ...
    MEB_cursor += mods_titleInfo.length;
    mods_name_01  =get_MODS_element_block3('mods:name','type','<mods:originInfo',chompStr,MEB_cursor);
    MEB_cursor += mods_name_01.length;
    mods_name_02  =get_MODS_element_block3('mods:name','type','<mods:originInfo',chompStr,MEB_cursor);
    MEB_cursor += mods_name_02.length;
    mods_name_03  =get_MODS_element_block3('mods:name','type','<mods:originInfo',chompStr,MEB_cursor);
    MEB_cursor += mods_name_03.length;
...
alert("testData3 get_MODS_element_block3: mods elements are complete");
   var genre_attribute = get_MODS_attribute(mods_genre,'authority');

   alert("genre_attribute="+genre_attribute);

   alert("mods_location=\n"+mods_location);
   //var location_attribute = get_MODS_attribute('mods_location','type','Number');
   //alert("location_attribute="+location_attribute);
   var test = get_MODS_entity(mods_location,"mods:physicalLocation","type","Number");
   alert("test=\n"+test);

   alert("test attribute:\n"+get_MODS_attribute(test,"type"));

   alert("name_01 type attribute:\n"+ get_MODS_attribute(mods_name_01,"type"));
   alert("name_01 authority attribute:\n"+ get_MODS_attribute(mods_name_01,"authority"));
   alert("name_02 authority attribute:\n"+ get_MODS_attribute(mods_name_02,"authority"));
   alert("name_03 authority attribute:\n"+ get_MODS_attribute(mods_name_03,"authority"));
...
//get level2 entity from top level entity 
//for example get mods:physicalLocation from mods_location
//need attr and attr_hint because mods:physicalLocation has 6 different type= things
function get_MODS_entity(mods_element_string,subelementName,attr,attr_hint) {

var e = new String (mods_element_string); //built by function get_MODS_element_block3
var s = new String (subelementName);
var a = new String (attr);
var h = new String (attr_hint);

var ts = new String ("");//test string

alert("e="+e+"\ns="+s+"\na="+a+"\nh="+h);

e = e.replace(/<mods:/g,":::<mods:");

eA = new Array();
eA = e.split(":::");
for (i=0;i<eA.length;i++) {
	ts = eA[i].substring(eA[i].indexOf(a),eA[i].indexOf(">"));
	alert("eA["+i+"]="+eA[i]+"\nts="+ts);

	if( ts.indexOf(attr_hint)>-1){
		return(eA[i]);
		}
	}
...
}//end function get_MODS_entity


//get attribute from top level entity
function get_MODS_attribute(mods_str,mods_attr) {

var s = new String (mods_str);
var a = new String (mods_attr);
...
var startTag = new String("<"+elementName);
	if ( attribute != "x" ) { startTag = startTag+" "+attribute; }
...
var startTag = new String(StartTag);
var endTag   = new String(EndTag);
var sentinelTag = new String(SentinelTag);
	//alert("got to  here startTag="+startTag);
...
return(MEB);
}//end function get_MODS_element_block2

//return entire MODS Element Block
function get_MODS_element_block(startTag,endTag,fromString,startIndex){

var S = new String(startTag);
    S = S.replace(">","");
    
var E = new String(endTag);
    E = E.replace(">","");
var B = new String(fromString);
var F = new String(); //workspace


var S = new String(startTag);
var E = new String(endTag);
var startMEB = B.indexOf(S);
if (startMEB == -1) return("");//missing tag, no harm done
//var endMEB = F.indexOf(E)+E.length+S.length;

var F = new String();
    F = B.substring(S.length); //prevent overlap
var startF = F.indexOf(">")+1; //this is to get past ">" on startTag that was removed
    F = F.substring(startF);
//var endMEB = F.indexOf(">")+S.length+startF+1; // find the end of the endTag, add length of S+startF
    

var endMEB = B.indexOf(E)+E.length;


//MEB MODS element block
var MEB = new String(B.substring(startMEB,endMEB));

alert("startIndex="+startIndex+"\nstartMEB="+startMEB+"\nendMEB="+endMEB+"\nS="+S+"\nE="+E
+"\n\n"+startTag+" MEB=\n\n"+MEB +"\n\n MEB.length="+MEB.length);


return(MEB);


}//end function getMODS_element(startTag)
...
}else if (    B.indexOf("Created by Volunteer Voices Workbook")<0 
	   && B.indexOf("Created by Special Collections Workbook") <0 ){//style B
