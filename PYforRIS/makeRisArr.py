...
#THIS IS CUSTOM FOR ARR DATA


#command line:
#python makeRisArr.py source_dir  target_dir >> redirectText
#python makeRisArr.py arrIN       arrOUT     >> optionalLog.txt

#use a log file instead of print to screen.

#commonalities for arr files

# TY  - FIGURE  because <dc:type>Image</dc:type>
# TI  - <dc:title>...</dc:title>
# PY  - 2009  
# CY  - (source) The University of Tennessee Digital Collections #constant value
# PB  - (Collection Name):  From Pi Beta Phi to Arrowmont 
# AU  - (Authors):  default value Anonymous
...
testTitlePost = "" ## " - 130807 docBlock6 ARR "
...
for item in Flist:
	...
	for item in Flines:
		...
		if s0.find("<dc:date>")>-1:
			...
			s4 = s3.rstrip(' ') #Y1
			displayY1 = s4
			s5 = s4.replace('May - June ','')
			s6 = s5.replace('Winter ','')
			s7 = s6.replace('Spring ','')
			s8 = s7.replace('Fall ','') #PY	
			s9 = s8
			i8 = s8.find("-")
			if i8>-1:
				s9=s8[0:i8]
			...
			myDateY1  ="\nY1  - "+ displayY1+ " "
			myDatePY  ="\nPY  - "+ s9+ " "
			dateOfPhotoCreation = "<li>Date of Document Creation: "+displayY1+"</li>" #docBlock
			...
			s4 = s3.rstrip(' ')
			s5 = s4
			if s4.find('Unknown')> -1: #correct per Anne
				s5= "Anonymous  "	
				#
				#
			myAuthor = '\nAU  - '+s5
			...
			if s0.find("<dc:description>The University of Tennessee")<0:
				...
				#
				#
		elif s0.find("<dc:relation>")>-1:
			if s0.find("<dc:relation>Bibliographic Citation:")>-1:
				ctBibCitation=1
				s1 = s0.replace("<dc:relation>","")
				s2 = s1.replace("</dc:relation>\n","")
				s3 = s2.lstrip(" ")
				docBibCitation = '<li>'+s3+'</li>'  #docBlock
				print filename+"\ndocBiblCitation: "+docBibCitation
				#
				#
		elif s0.find("<dc:coverage>")>-1:
			s1 = s0.replace("<dc:coverage>","")
			s2 = s1.replace("</dc:coverage>\n","")
			s3 = s2.lstrip(" ")
			docCoverage = '<li>Coverage: '+s3+'</li>'  #docBlock
	...
	#myKeywords = myK.replace('\nKW  - ','',1)
        ...
        #constants for PPI############################
	myRisType   = "\nTY  - FIGURE " # all arr are images
	mySource    = "\nCY  - The Univerisity of Tennessee Libraries Digital Collections "  #constant
	myPublisher = '\nPB  - From Pi Beta Phi to Arrowmont  '
	...
	if ctDescription<1: #special case since only using one kind of <dc:description>
		...
		#
		#
	if ctBibCitation<1: #special case since only using one kind of <dc:relation>
		docBibCitation = "" #for when it is missing
		#
		#
	if Fall.find("<dc:identifier>TU")<0:
		...
		#
		#
	if Fall.find("<dc:coverage>")<0:
		docCoverage   = ""
		#
		#

	docDates = dateOfPhotoCreation ###+dateOfDigitalPublication
	docBlock  = '\nN2  - <ul>'+docCoverage+docBibCitation+docOrigFormat+docDates+docDocId+'</ul>'
