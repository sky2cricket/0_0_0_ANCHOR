...
#Special Request from Ken and Anne instead of digital publication year.
#PY publication year 1886: 

#command line:
#python makeRisWcc.py source_dir  target_dir >> redirectText
#python makeRisWcc.py wccIN       wccOUT     >> optionalLog.txt

#use a log file instead of print to screen.

#commonalities for wcc files
...
testTitlePost = "" ###" - 1300806 docBlock6 WCC "
...
for item in Flist:
	...
	for item in Flines:
		...
		if s0.find("<dc:date>")>-1:
			...
			#<dc:source>MS 552 ( http://idserver.utk.edu/?id=200600000000261 );  William Cox Cochran Great Smoky Mountains Photographic Collection, 1886;  Special Collections Library, The University of Tennessee</dc:source>
			s1 = s0.replace("<dc:source>","")
			s2 = s1.replace("</dc:source>\n","")
			s3 = s2.replace(" ( http://idserver.utk.edu/?id=200600000000261 )","")
			s4 = s3.lstrip(" ")
			docOrigFormat ='<li>Original Format: '+s4+'</li>'  #docBlock
	...
	#myKeywords = myK.replace('\nKW  - ','',1)
        ...
        #constants for WCC############################
	myRisType   = "\nTY  - FIGURE " # all wcc are images
	mySource    = "\nCY  - The University of Tennessee Libraries Digital Collections "  #constant
	myPublisher = '\nPB  - William Cox Cochran Great Smoky Mountains Photographic Collection '
	myAuthor = "\nAU  - William Cox Cochran "
