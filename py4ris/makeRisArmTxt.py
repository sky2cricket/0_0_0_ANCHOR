...
#THIS IS CUSTOM FOR ARMTXT DATA
#Special processing: 
...
#command line:
#python makeRisArmTxt.py source_dir  target_dir >> redirectText
#python makeRisArmTxt.py armtxtIN    armtxtOUT  >> optionalLog.txt

#use a log file instead of print to screen.

#commonalities for armTxt files
...
testTitlePost = "" ##" - 130807 docBlock6 ARMTXT "
...
for item in Flist:
	...
	for item in Flines:
		...
		if s0.find("<dc:date>")>-1:
			...
			if len(s4)>2:
				...
				myDateY1 = "\nY1  - "+s5yr+"  "
				myDatePY = "\nPY  - "+s5yr +"  "
			dateOfPhotoCreation = "<li>Date of Document Creation: "+s3a+"</li>" #docBlock
			...
			s4 = s3.rstrip(' ')
			s5 = s4
			if s4.find('Unknown')>-1: #correction per Anne
				s5= "Anonymous  "	
				#
				#
			myAuthor = '\nAU  - '+s5
			...
			if s0.find("<dc:description>The University of Tennessee")<0:
				...
				#
				#
		elif s0.find("<dc:format>")>-1:
			#<dc:format>Text/html</dc:format>
			if s0.find("<dc:format>Text")<0:
				ctDocsize=1
				s1 = s0.replace("<dc:format>","")
				s2 = s1.replace("</dc:format>\n","")
				s3 = s2.lstrip(" ")
				docPages = "<li>Document Extent: "+s3+"</li>" #docBlock
	...
	#myKeywords = myK.replace('\nKW  - ','',1)
        ...
        #constants for PPI############################
	myRisType   = "\nTY  - FIGURE " # all armtxt are images
	mySource    = "\nCY  - The University of Tennessee Libraries Digital Collectionss "  #constant
	...
	if ctDescription<1: #special case since only using one kind of <dc:description>
		...
		#
		#
	if ctDocsize <1: #special case only using one kind of <dc:format>
		docPages = "" #for when it is missing
		#
		#
	...
	docBlock  = '\nN2  - <ul>'+docOrigFormat+docPages+docDates+docDocId+'</ul>'
