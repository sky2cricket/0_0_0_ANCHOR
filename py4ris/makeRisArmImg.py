...
#THIS IS CUSTOM FOR ARMIMG DATA
#Special processing: 
...
#command line:
#python makeRisArmImg.py source_dir  target_dir >> redirectText
#python makeRisArmImg.py armimgIN    armimgOUT  >> optionalLog.txt

#use a log file instead of print to screen.

#commonalities for armImg files
...
testTitlePost = "" ##" - 130807 docBlock6 ARMIMG "
...
for item in Flist:
	...
	for item in Flines:
		...
		if s0.find("<dc:date>")>-1:
			...
			s4 = s3.rstrip(' ')
			s5 = s4
			if s4.find('Unknown')>-1: #change per Anne
				s5= "Anonymous"	
				#
				#
			myAuthor = '\nAU  - '+s5
	...
	#myKeywords = myK.replace('\nKW  - ','',1)
        ...
        #constants for PPI############################
	myRisType   = "\nTY  - FIGURE " # all armimg are images
