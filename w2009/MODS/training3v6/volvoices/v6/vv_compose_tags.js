...
function buildTagsArray_header(N){
	...
	var commentReloadDate  = new String(parent.U.document.UF.VVcomment_03r.value);
...
//function form_filter called from buildModsTags on each page to get rid of out-of-range characters in
//form values coming in from ADMIN DB or other cut and paste activity
//process each form element that is type "text" or type "textarea"
//the function vv_filter is in the file vv_filter.js
function form_filter() {
	for(i=0;i<parent.D.document.DF.length;i++) {
		if ( parent.D.document.DF.elements[i].type == "text" 
		  || parent.D.document.DF.elements[i].type == "textarea" ) {
		parent.D.document.DF.elements[i].value = parent.L.vv_filter(parent.D.document.DF.elements[i].value);
		}//end if
	}//end i for
}///end function form_filter()
